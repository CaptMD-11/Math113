...
\begin{enumerate}
	...
	\item \textbf{Prop. } Take $G = \mathbb{Z}$, $H = n\mathbb{Z}$, $a,b \in \mathbb{Z}$. Then $aH \otimes bH$ gives $(a+n\mathbb{Z}) \otimes (b+n\mathbb{Z}) = (a + b) + n\mathbb{Z}$ is correctly defined. 
	\item \textbf{Theorem. } Let $G$ be a group and $H$ a normal subgroup. Then $\otimes$ (as in the above Prop.) defines a group structure on $G/H$, where $G/H$ is called a quotient (factor) group. 
